...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@H :{}@HH31 0 0'.format(vxi, vxi)).execute()
                ...
                change(parm, 'charge', ':{}@HA2'.format(vxi), bc['HA2']+((fc['HA2']-bc['HA2'])/10)*i).execute()
                change(parm, 'charge', ':{}@HA3'.format(vxi), bc['HA3']+((fc['HA3']-bc['HA3'])/10)*i).execute()
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	hydhyd = var[2]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/GLY-SAR.pdb\n"%vxi)
