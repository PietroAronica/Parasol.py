...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/B1D.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@OD1'.format(vxi), bc['OD1']+((fc['OD1']-bc['OD1'])/10)*i).execute()
                change(parm, 'charge', ':{}@OD2'.format(vxi), bc['OD2']+((fc['OD2']-bc['OD2'])/10)*i).execute()
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	ih = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ASP-B1D.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.11 name "CA2"\n'%vxi)
	ctrl.write('set %s.1.12 name "HA22"\n'%vxi)
	ctrl.write('set %s.1.13 name "HA23"\n'%vxi)
	...
	ctrl.write('set %s.1.5 type "2C"\n'%vxi)
	ctrl.write('set %s.1.6 type "HC"\n'%vxi)
	ctrl.write('set %s.1.7 type "HC"\n'%vxi)
	ctrl.write('set %s.1.8 type "CO"\n'%vxi)
	ctrl.write('set %s.1.9 type "O2"\n'%vxi)
	ctrl.write('set %s.1.10 type "O2"\n'%vxi)
...
def stock_add_to_all(var=variablemake()):
	...
	for i in range(11):
		...
		Frcmod_creator.ANGLE_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}'.format('2C', 'CX', ic), cal(p['CT_CT_C'][0], p['C_C_C'][0], i), cal(p['CT_CT_C'][1], p['C_C_C'][1], i))
		...
		Frcmod_creator.ANGLE_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}'.format(ic, 'C ', 'N '), cal(p['C_C_N'][0], p['C_C_N'][0], i), cal(p['C_C_N'][1], p['C_C_N'][1], i))
                ...
                Frcmod_creator.DIHEDRAL_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}-{}'.format('2C', 'CX', ic, 'C '), cal(p['0_4'][0], p['X_C_C_X'][0], i), cal(p['0_4'][1], p['X_C_C_X'][1], i), cal(p['0_4'][2], p['X_C_C_X'][2], i), cal(p['0_4'][3], p['X_C_C_X'][3], i))
