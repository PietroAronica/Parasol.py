...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        with open('Param_files/AminoAcid/X2X.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@C2'.format(vxi), (fc['C2']/10)*i).execute()
                change(parm, 'charge', ':{}@H22'.format(vxi), (fc['H22']/10)*i).execute()
                change(parm, 'charge', ':{}@H23'.format(vxi), (fc['H23']/10)*i).execute()
                change(parm, 'charge', ':{}@C3'.format(vxi), bc['C2']+((fc['C3']-bc['C2'])/10)*i).execute()
                change(parm, 'charge', ':{}@H32'.format(vxi), bc['H22']+((fc['H32']-bc['H22'])/10)*i).execute()
                change(parm, 'charge', ':{}@H33'.format(vxi), bc['H23']+((fc['H33']-bc['H23'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'H13' and res.get_resname() == vxi:
                        	pdb.write(atom.formatted())
                        	pdb.write(atom.halfway_between('C2', C1, C2))
                        	pdb.write(atom.superimposed1('H22', C2))
                        	pdb.write(atom.superimposed1('H23', C2))
			elif atom.get_name() == 'C2' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('C3'))
			elif atom.get_name() == 'H22' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H32'))
			elif atom.get_name() == 'H23' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H33'))
			else:
                        	pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
 	...
 	inthyd = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/X2X-X3X.pdb\n"%vxi)
	...
	ctrl.write('set %s.1 connect1 %s.1.C3\n'%(vxi, vxi))
