...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/X8X.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@C7'.format(vxi), (fc['C7']/10)*i).execute()
                change(parm, 'charge', ':{}@H72'.format(vxi), (fc['H72']/10)*i).execute()
                change(parm, 'charge', ':{}@H73'.format(vxi), (fc['H73']/10)*i).execute()
                change(parm, 'charge', ':{}@C8'.format(vxi), bc['C7']+((fc['C8']-bc['C7'])/10)*i).execute()
                change(parm, 'charge', ':{}@H82'.format(vxi), bc['H72']+((fc['H82']-bc['H72'])/10)*i).execute()
                change(parm, 'charge', ':{}@H83'.format(vxi), bc['H73']+((fc['H83']-bc['H73'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
        ...
        C7 = struct.residue_dict[aa].atom_dict['C7']
	pdb = open(out, 'w')
        try:
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'H63' and res.get_resname() == vxi:
                        	pdb.write(atom.formatted())
                        	pdb.write(atom.halfway_between('C7', C6, C7))
                        	pdb.write(atom.superimposed1('H72', C7))
                        	pdb.write(atom.superimposed1('H73', C7))
			elif atom.get_name() == 'C7' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('C8'))
			elif atom.get_name() == 'H72' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H82'))
			elif atom.get_name() == 'H73' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H83'))
			else:
                        	pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
 	...
 	inthyd = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/X7X-X8X.pdb\n"%vxi)
	...
	ctrl.write('set %s.1 connect1 %s.1.C8\n'%(vxi, vxi))
	ctrl.write('set %s name "%s"\n'%(vxi, vxi))
	ctrl.write('set %s.1 name "%s"\n'%(vxi, vxi))
	ctrl.write('set %s head %s.1.C1\n'%(vxi, vxi))
	ctrl.write('set %s tail %s.1.C8\n'%(vxi, vxi))
