...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/B1S.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@OG'.format(vxi), bc['OG']+((fc['OG']-bc['OG'])/10)*i).execute()
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	ih = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/SER-B1S.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.10 name "CA2"\n'%vxi)
	ctrl.write('set %s.1.11 name "HA22"\n'%vxi)
	ctrl.write('set %s.1.12 name "HA23"\n'%vxi)
