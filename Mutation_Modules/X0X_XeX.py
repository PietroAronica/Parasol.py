...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/XeX.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@C9'.format(vxi), bc['C9']+((fc['C9']-bc['C9'])/10)*i).execute()
                change(parm, 'charge', ':{}@H92'.format(vxi), bc['H92']+((fc['H92']-bc['H92'])/10)*i).execute()
                change(parm, 'charge', ':{}@H93'.format(vxi), bc['H93']+((fc['H93']-bc['H93'])/10)*i).execute()
                change(parm, 'charge', ':{}@C10'.format(vxi), (fc['C10']/10)*i).execute()
                change(parm, 'charge', ':{}@H102'.format(vxi), (fc['H102']/10)*i).execute()
                change(parm, 'charge', ':{}@H103'.format(vxi), (fc['H103']/10)*i).execute()
                change(parm, 'charge', ':{}@C11'.format(vxi), bc['C10']+((fc['C11']-bc['C10'])/10)*i).execute()
                change(parm, 'charge', ':{}@H112'.format(vxi), bc['H102']+((fc['H112']-bc['H102'])/10)*i).execute()
                change(parm, 'charge', ':{}@H113'.format(vxi), bc['H103']+((fc['H113']-bc['H103'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
        ...
        C10 = struct.residue_dict[aa].atom_dict['C10']
	pdb = open(out, 'w')
        try:
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'H93' and res.get_resname() == vxi:
                        	pdb.write(atom.formatted())
                        	pdb.write(atom.halfway_between('C10', C9, C10))
                        	pdb.write(atom.superimposed1('H102', C10))
                        	pdb.write(atom.superimposed2('H103', C10))
			elif atom.get_name() == 'C10' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('C11'))
			elif atom.get_name() == 'H102' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H112'))
			elif atom.get_name() == 'H103' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H113'))
			else:
                        	pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
 	...
 	inthyd = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/X0X-XeX.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.32 element "H"\n'%vxi)
	...
	ctrl.write('set %s.1.31 name "C11"\n'%vxi)
	ctrl.write('set %s.1.32 name "H112"\n'%vxi)
	ctrl.write('set %s.1.33 name "H113"\n'%vxi)
	...
	ctrl.write('set %s.1.31 type "CT"\n'%vxi)
	ctrl.write('set %s.1.32 type "HC"\n'%vxi)
	ctrl.write('set %s.1.33 type "HC"\n'%vxi)
