...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@CB1'.format(vxi), (fc['CB1']/10)*i).execute()
                change(parm, 'charge', ':{}@HB12'.format(vxi), (fc['HB12']/10)*i).execute()
                change(parm, 'charge', ':{}@HB13'.format(vxi), (fc['HB13']/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
        ...
        CA1 = struct.residue_dict[aa].atom_dict['CA1']
        C1 = struct.residue_dict[aa].atom_dict['C1']
        CA2 = struct.residue_dict[aa].atom_dict['CA2']
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HA13' and res.get_resname() == vxi:
                        	pdb.write(atom.formatted())
                        	pdb.write(atom.halfway_between('CB1', CA1, C1))
                        	pdb.write(atom.superimposed1('HB12', CA1))
                        	pdb.write(atom.superimposed1('HB13', CA1))
			elif atom.get_name() == 'H22' and res.get_resname() == vxi:
                        	pdb.write(atom.formatted())
                        	pdb.write(atom.halfway_between('CB2', CA2, C4))
                        	pdb.write(atom.superimposed1('HB22', CA2))
                        	pdb.write(atom.superimposed1('HB23', CA2))
			else:
                        	pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
 	...
 	inthyd = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/XPX-XXX.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.7  name "C1"\n'%vxi)
	ctrl.write('set %s.1.8  name "C21"\n'%vxi)
	ctrl.write('set %s.1.9  name "H21"\n'%vxi)
	ctrl.write('set %s.1.10 name "C31"\n'%vxi)
	ctrl.write('set %s.1.11 name "H31"\n'%vxi)
	ctrl.write('set %s.1.12 name "C4"\n'%vxi)
	ctrl.write('set %s.1.13 name "C32"\n'%vxi)
	ctrl.write('set %s.1.14 name "H32"\n'%vxi)
	ctrl.write('set %s.1.15 name "C22"\n'%vxi)
	ctrl.write('set %s.1.16 name "H22"\n'%vxi)
	ctrl.write('set %s.1.17 name "CB2"\n'%vxi)
	ctrl.write('set %s.1.18 name "HB22"\n'%vxi)
	ctrl.write('set %s.1.19 name "HB23"\n'%vxi)
	ctrl.write('set %s.1.20 name "CA2"\n'%vxi)
	ctrl.write('set %s.1.21 name "HA22"\n'%vxi)
	ctrl.write('set %s.1.22 name "HA23"\n'%vxi)
	...
	ctrl.write('set %s.1.7  type "CA"\n'%vxi)
	ctrl.write('set %s.1.8  type "CA"\n'%vxi)
	ctrl.write('set %s.1.9  type "HA"\n'%vxi)
	ctrl.write('set %s.1.10 type "CA"\n'%vxi)
	ctrl.write('set %s.1.11 type "HA"\n'%vxi)
