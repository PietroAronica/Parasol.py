...
def parmed_command(vxi='VXI'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                changeLJPair(parm, ':{}@HG2 :{}@OE1 0 0'.format(vxi, vxi)).execute()
                ...
                change(parm, 'charge', ':{}@CD1'.format(vxi), (bc['CD']-(bc['CD']/10)*i)*((10-i)*(10-i))/100).execute()
                change(parm, 'charge', ':{}@OE1'.format(vxi), (bc['OE1']-(bc['OE1']/10)*i)*((10-i)*(10-i))/100).execute()
                change(parm, 'charge', ':{}@NE2'.format(vxi), (bc['NE2']-(bc['NE2']/10)*i)*((10-i)*(10-i))/100).execute()
                change(parm, 'charge', ':{}@HE21'.format(vxi), (bc['HE21']-(bc['HE21']/10)*i)*((10-i)*(10-i))/100).execute()
                change(parm, 'charge', ':{}@HE22'.format(vxi), (bc['HE22']-(bc['HE22']/10)*i)*((10-i)*(10-i))/100).execute()
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HG2' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.halfway_between('CD', CG, HG2))
                                ...
                                pdb.write(atom.superimposed3('HD3', HG2))
			else:
                        	pdb.write(atom.formatted())
                try:
...
def lib_make(ff, outputfile, vxi='VXI', metcar='1c', methyd='1h', hydhyd1='xh', amicar='ac', amioxy='ao', aminit='an', amihyd='ah', hydhyd2='sh'):
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
...
def stock_add_to_all(vxi='VXI', metcar='1c', methyd='1h', hydhyd1='xh', amicar='ac', amioxy='ao', aminit='an', amihyd='ah', hydhyd2='sh'):
	...
	for i in range(11):
		...
		Frcmod_creator.ANGLE_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}'.format(hydhyd2, 'CT', hydhyd1), cal(p['H_C_H'][0], p['H_C_H'][0], i), cal(p['H_C_H'][1], p['H_C_H'][1], i))
