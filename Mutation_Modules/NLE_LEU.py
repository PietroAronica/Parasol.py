...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HG2'.format(vxi), ':{}@HD21'.format(vxi), '0', '0').execute()
		changeLJPair(parm, ':{}@HD11'.format(vxi), ':{}@HE1'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@HG'.format(vxi), bc['HG3']+((fc['HG']-bc['HG3'])/10)*i).execute()
                change(parm, 'charge', ':{}@CD1'.format(vxi), bc['CD']+((fc['CD1']-bc['CD'])/10)*i).execute()
                change(parm, 'charge', ':{}@HD11'.format(vxi), (fc['HD11']/10)*i).execute()
                change(parm, 'charge', ':{}@HD12'.format(vxi), bc['HD2']+((fc['HD12']-bc['HD2'])/10)*i).execute()
                change(parm, 'charge', ':{}@HD13'.format(vxi), bc['HD3']+((fc['HD13']-bc['HD3'])/10)*i).execute()
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HG3' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.superimposed1('HD11', CE))
                        	...
                        	pdb.write(atom.halfway_between('CD2', CG, HG2))
                        	pdb.write(atom.superimposed1('HD21', HG2))
                        	pdb.write(atom.superimposed2('HD22', HG2))
                        	pdb.write(atom.superimposed3('HD23', HG2))
			else:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	hydhyd2 = var[5]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/LEU-NLE.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.14 name "HD13"\n'%vxi)
	ctrl.write('set %s.1.15 name "CD2"\n'%vxi)
	ctrl.write('set %s.1.16 name "HD21"\n'%vxi)
	ctrl.write('set %s.1.17 name "HD22"\n'%vxi)
	ctrl.write('set %s.1.18 name "HD23"\n'%vxi)
