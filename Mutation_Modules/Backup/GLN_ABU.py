	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'CG' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.superimposed1('HG1', CD))
			else:
                        	pdb.write(atom.formatted())
                try:
...
def stock_add_to_all(amicar='ac', amioxy='ao', aminit='an', amihyd='ah', hydhyd='sh'):
	...
	for i in range(11):
		...
		Frcmod_creator.BOND_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}'.format(amicar, amioxy), cal(p['C_O2'][0], p['O2_mH'][0], i), cal(p['C_O2'][1], p['O2_mH'][1], i))
		...
		Frcmod_creator.ANGLE_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}'.format('CT', amicar, amioxy), cal(p['C_C_O2'][0], p['Dritt'][0], i), cal(p['C_C_O2'][1], p['Dritt'][1], i))
