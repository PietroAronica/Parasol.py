...
def parmed_command(vxi='VXI'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@CG1'.format(vxi), bc['CG2']+((fc['CG1']-bc['CG2'])/10)*i).execute()
                change(parm, 'charge', ':{}@HG11'.format(vxi), bc['HG21']-(bc['HG21']/10)*i).execute()
                change(parm, 'charge', ':{}@HG12'.format(vxi), bc['HG22']+((fc['HG12']-bc['HG22'])/10)*i).execute()
                change(parm, 'charge', ':{}@HG13'.format(vxi), bc['HG23']+((fc['HG13']-bc['HG23'])/10)*i).execute()
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HB' and res.get_resname() == vxi:
                        	pdb.write(atom.superimposed1('HB', OG1))
                        	pdb.write(atom.change_name('HB2'))
                        	pdb.write(atom.halfway_between('CG2', CB, HB))
                        	pdb.write(atom.superimposed1('HG21', HB))
                        	pdb.write(atom.superimposed2('HG22', HB))
                        	pdb.write(atom.superimposed3('HG23', HB))
