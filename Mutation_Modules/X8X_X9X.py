...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@C8'.format(vxi), (fc['C8']/10)*i).execute()
                change(parm, 'charge', ':{}@H82'.format(vxi), (fc['H82']/10)*i).execute()
                change(parm, 'charge', ':{}@H83'.format(vxi), (fc['H83']/10)*i).execute()
                change(parm, 'charge', ':{}@C9'.format(vxi), bc['C8']+((fc['C9']-bc['C8'])/10)*i).execute()
                change(parm, 'charge', ':{}@H92'.format(vxi), bc['H82']+((fc['H92']-bc['H82'])/10)*i).execute()
                change(parm, 'charge', ':{}@H93'.format(vxi), bc['H83']+((fc['H93']-bc['H83'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'H73' and res.get_resname() == vxi:
                        	pdb.write(atom.formatted())
                        	pdb.write(atom.halfway_between('C8', C7, C8))
                        	pdb.write(atom.superimposed1('H82', C8))
                        	pdb.write(atom.superimposed2('H83', C8))
			elif atom.get_name() == 'C8' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('C9'))
			elif atom.get_name() == 'H82' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H92'))
			elif atom.get_name() == 'H83' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H93'))
			else:
                        	pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
 	...
 	inthyd = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/X8X-X9X.pdb\n"%vxi)
	...
	ctrl.write('set %s.1 connect1 %s.1.C9\n'%(vxi, vxi))
	ctrl.write('set %s name "%s"\n'%(vxi, vxi))
	ctrl.write('set %s.1 name "%s"\n'%(vxi, vxi))
	ctrl.write('set %s head %s.1.C1\n'%(vxi, vxi))
	ctrl.write('set %s tail %s.1.C9\n'%(vxi, vxi))
