...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HB3 :{}@HG21 0 0'.format(vxi, vxi)).execute()
                ...
                change(parm, 'charge', ':{}@HG1'.format(vxi), bc['HG11']+((fc['HG1']-bc['HG11'])/10)*i).execute()
                ...
                change(parm, 'charge', ':{}@HG23'.format(vxi), bc['HG22']-(bc['HG23']/10)*i).execute()
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	hydhyd = var[2]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ABU-VAL.pdb\n"%vxi)
