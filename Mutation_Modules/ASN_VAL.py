...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HB'.format(vxi), ':{}@OD1'.format(vxi), '0', '0').execute()
		changeLJPair(parm, ':{}@HB'.format(vxi), ':{}@ND2'.format(vxi), '0', '0').execute()
		changeLJPair(parm, ':{}@HB'.format(vxi), ':{}@HD21'.format(vxi), '0', '0').execute()
		...
		changeLJPair(parm, ':{}@HB3'.format(vxi), ':{}@HG21'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@CG'.format(vxi), (bc['CG']-(bc['CG']/10)*i)*(10-i)/10).execute()
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HB2' and res.get_resname() == vxi:
                        	pdb.write(atom.superimposed1('HB', CG))
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	hydhyd3 = var[10]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/VAL-ASN.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.19 name "ND2"\n'%vxi)
