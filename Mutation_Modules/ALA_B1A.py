...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/B1A.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@HB1'.format(vxi), bc['HB1']+((fc['HB1']-bc['HB1'])/10)*i).execute()
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	ih = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ALA-B1A.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.9 name "CA2"\n'%vxi)
	ctrl.write('set %s.1.10 name "HA22"\n'%vxi)
	ctrl.write('set %s.1.11 name "HA23"\n'%vxi)
