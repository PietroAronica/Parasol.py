...
def parmed_command(vxi='VXI'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                changeLJPair(parm, ':{}@HG21 :{}@HD11 0 0'.format(vxi, vxi)).execute()
                ...
                change(parm, 'charge', ':{}@HB2'.format(vxi), bc['HB']+(bc['HB'])/10*i).execute()
                change(parm, 'charge', ':{}@CG1'.format(vxi), bc['CG2']-(bc['CG2']/10)*i).execute()
                change(parm, 'charge', ':{}@HG11'.format(vxi), bc['HG21']-(bc['HG21']/10)*i).execute()
                change(parm, 'charge', ':{}@HG12'.format(vxi), bc['HG22']-(bc['HG22']/10)*i).execute()
                change(parm, 'charge', ':{}@HG13'.format(vxi), bc['HG23']-(bc['HG23']/10)*i).execute()
                change(parm, 'charge', ':{}@CG2'.format(vxi), bc['CG1']+((fc['CG2']-bc['CG1'])/10)*i).execute()
                change(parm, 'charge', ':{}@HG21'.format(vxi), (fc['HG21']/10)*i).execute()
                change(parm, 'charge', ':{}@HG22'.format(vxi), bc['HG12']+((fc['HG22']-bc['HG12'])/10)*i).execute()
                change(parm, 'charge', ':{}@HG23'.format(vxi), bc['HG13']+((fc['HG23']-bc['HG13'])/10)*i).execute()
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HB' and res.get_resname() == vxi:
                        	pdb.write(atom.superimposed1('HB', CG2))
                        	...
                        	pdb.write(atom.superimposed1('HG21', CD1))
...
def lib_make(ff, outputfile, vxi='VXI', metcar1='1c', methyd1='1h', hydhyd1='xh', metcar2='2c', methyd2='2h', hydhyd2='yh', alcoxy='ho', alchyd='hh', hydhyd3='sh'):
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ILE-THR.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.18 name "CD1"\n'%vxi)
	ctrl.write('set %s.1.19 name "HD11"\n'%vxi)
	ctrl.write('set %s.1.20 name "HD12"\n'%vxi)
	ctrl.write('set %s.1.21 name "HD13"\n'%vxi)
...
def stock_add_to_all(vxi='VXI', metcar1='1c', methyd1='1h', hydhyd1='xh', metcar2='2c', methyd2='2h', hydhyd2='yh', alcoxy='ho', alchyd='hh', hydhyd3='sh'):
