...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        with open('Param_files/AminoAcid/BAA.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@CA1'.format(vxi), ':{}@H1'.format(vxi), '0', '0').execute()
		changeLJPair(parm, ':{}@O'.format(vxi), ':{}@H1'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@CA1'.format(vxi), bc['CA1']).execute()
                change(parm, 'charge', ':{}@HA12'.format(vxi), bc['HA12']).execute()
                change(parm, 'charge', ':{}@HA13'.format(vxi), bc['HA13']).execute()
                change(parm, 'charge', ':{}@CA2'.format(vxi), bc['CA2']).execute()
                change(parm, 'charge', ':{}@HA22'.format(vxi), bc['HA22']).execute()
                change(parm, 'charge', ':{}@HA23'.format(vxi), bc['HA23']).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
        ...
        CA = struct.residue_dict[aa].atom_dict['CA1']
	pdb = open(out, 'w')
        try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	newoxy = var[8]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/GLY_X_BAA.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.10 name "CA1"\n'%vxi)
	ctrl.write('set %s.1.11 name "HA12"\n'%vxi)
	ctrl.write('set %s.1.12 name "HA13"\n'%vxi)
