...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@C4'.format(vxi), (fc['C4']/10)*i).execute()
                change(parm, 'charge', ':{}@H42'.format(vxi), (fc['H42']/10)*i).execute()
                change(parm, 'charge', ':{}@H43'.format(vxi), (fc['H43']/10)*i).execute()
                change(parm, 'charge', ':{}@C5'.format(vxi), bc['C4']+((fc['C5']-bc['C4'])/10)*i).execute()
                change(parm, 'charge', ':{}@H52'.format(vxi), bc['H42']+((fc['H52']-bc['H42'])/10)*i).execute()
                change(parm, 'charge', ':{}@H53'.format(vxi), bc['H43']+((fc['H53']-bc['H43'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'H33' and res.get_resname() == vxi:
                        	pdb.write(atom.formatted())
                        	pdb.write(atom.halfway_between('C4', C3, C4))
                        	pdb.write(atom.superimposed1('H42', C4))
                        	pdb.write(atom.superimposed1('H43', C4))
			elif atom.get_name() == 'C4' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('C5'))
			elif atom.get_name() == 'H42' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H52'))
			elif atom.get_name() == 'H43' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H53'))
			else:
                        	pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
 	...
 	inthyd = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/X4X-X5X.pdb\n"%vxi)
	...
	ctrl.write('set %s.1 connect1 %s.1.C5\n'%(vxi, vxi))
