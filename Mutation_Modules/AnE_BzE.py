...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/BzE.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HE1'.format(vxi), ':{}@HE2'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@OT1'.format(vxi), bc['OH1']+((fc['OT1']-bc['OH1'])/10)*i).execute()
                change(parm, 'charge', ':{}@OT2'.format(vxi), bc['OH2']+((fc['OT2']-bc['OH2'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
	...
	pdb = open(out, 'w')
		...
		for atom in res.atom_list:
			...
			elif atom.get_name() == 'OH1' and res.get_resname() == vxi:
				pdb.write(atom.change_name('OT1')) 
			elif atom.get_name() == 'OH2' and res.get_resname() == vxi:
				pdb.write(atom.change_name('OT2')) 
			else:
				pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	he = var[2]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/AnE-BzE.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.18 name "CH"\n'%vxi)
	ctrl.write('set %s.1.19 name "OT1"\n'%vxi)
	ctrl.write('set %s.1.20 name "OT2"\n'%vxi)
...
def stock_add_to_all(var=variablemake()):
	...
	for i in range(11):
		...
		Frcmod_creator.ANGLE_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}'.format('C ', 'CA', ce), lac(p['F_CA_CA_HA'][0], p['A_CA_CA_HA'][0], i), lac(p['F_CA_CA_HA'][1], p['A_CA_CA_HA'][1], i))
		...
		Frcmod_creator.ANGLE_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}'.format('CT', 'CA', cd), lac(p['F_CT_CA_CA'][0], p['A_CT_CA_CA'][0], i), lac(p['F_CT_CA_CA'][1], p['A_CT_CA_CA'][1], i))
                ...
                Frcmod_creator.DIHEDRAL_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}-{}'.format(he, ce, 'CA', 'C '), lac(p['Ring_Dihe'][0], p['Ring_0'][0], i), lac(p['Ring_Dihe'][1], p['Ring_0'][1], i), lac(p['Ring_Dihe'][2], p['Ring_0'][2], i), lac(p['Ring_Dihe'][3], p['Ring_0'][3], i))
                Frcmod_creator.DIHEDRAL_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}-{}'.format(cd, ce, 'CA', 'C '), lac(p['Ring_Dihe'][0], p['Ring_0'][0], i), lac(p['Ring_Dihe'][1], p['Ring_0'][1], i), lac(p['Ring_Dihe'][2], p['Ring_0'][2], i), lac(p['Ring_Dihe'][3], p['Ring_0'][3], i))
