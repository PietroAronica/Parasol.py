...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HA2 :{}@HB1 0 0'.format(vxi, vxi)).execute()
                ...
                change(parm, 'charge', ':{}@HA2'.format(vxi), bc['HA2']-(bc['HA2']/10)*i).execute()
                change(parm, 'charge', ':{}@HA'.format(vxi), bc['HA3']+((fc['HA']-bc['HA3'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
        ...
        HA2 = struct.residue_dict[aa].atom_dict['HA2']
	pdb = open(out, 'w')
        try:
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HA2' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.halfway_between('CB', CA, HA2))
                        	pdb.write(atom.superimposed1('HB1', HA2))
                        	pdb.write(atom.superimposed2('HB2', HA2))
                        	pdb.write(atom.superimposed3('HB3', HA2))
