...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/AIB.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HA :{}@HB21 0 0'.format(vxi, vxi)).execute()
                ...
                change(parm, 'charge', ':{}@HB11'.format(vxi), bc['HB1']+((fc['HB11']-bc['HB1'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			elif atom.get_name() == 'HB1' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('HB11'))
...
def lib_make(ff, outputfile, vxi='VXI', metcar='cd', methyd='dh', hydhyd='mh'):
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ALA-AIB.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.10 name "HB11"\n'%vxi)
	ctrl.write('set %s.1.11 name "HB12"\n'%vxi)
	ctrl.write('set %s.1.12 name "HB13"\n'%vxi)
