...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@O'.format(vxi), ':{}@H1'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@OH'.format(vxi), bc['OH']).execute()
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	newoxy = var[8]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/GLY_X_TYR.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.18 name "CE1"\n'%vxi)
	ctrl.write('set %s.1.19 name "HE1"\n'%vxi)
	ctrl.write('set %s.1.20 name "CZ"\n'%vxi)
	ctrl.write('set %s.1.21 name "OH"\n'%vxi)
	...
	ctrl.write('set %s.1.21 type "OH"\n'%vxi)
	ctrl.write('set %s.1.22 type "HO"\n'%vxi)
