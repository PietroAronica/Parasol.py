...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        with open('Param_files/AminoAcid/X6X.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/X7X.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@C6'.format(vxi), (fc['C6']/10)*i).execute()
                change(parm, 'charge', ':{}@H62'.format(vxi), (fc['H62']/10)*i).execute()
                change(parm, 'charge', ':{}@H63'.format(vxi), (fc['H63']/10)*i).execute()
                change(parm, 'charge', ':{}@C7'.format(vxi), bc['C6']+((fc['C7']-bc['C6'])/10)*i).execute()
                change(parm, 'charge', ':{}@H72'.format(vxi), bc['H62']+((fc['H72']-bc['H62'])/10)*i).execute()
                change(parm, 'charge', ':{}@H73'.format(vxi), bc['H63']+((fc['H73']-bc['H63'])/10)*i).execute()
		setOverwrite(parm).execute()
		parmout(parm, 'Solv_{}_{}.prmtop'.format(a, 100-a)).execute()

def makevxi(struct, out, aa, vxi='VXI'):
        struct.residue_dict[aa].set_resname(vxi)
        C5 = struct.residue_dict[aa].atom_dict['C5']
        C6 = struct.residue_dict[aa].atom_dict['C6']
	pdb = open(out, 'w')
        try:
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'H53' and res.get_resname() == vxi:
                        	pdb.write(atom.formatted())
                        	pdb.write(atom.halfway_between('C6', C5, C6))
                        	pdb.write(atom.superimposed1('H62', C6))
                        	pdb.write(atom.superimposed1('H63', C6))
			elif atom.get_name() == 'C6' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('C7'))
			elif atom.get_name() == 'H62' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H72'))
			elif atom.get_name() == 'H63' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H73'))
			else:
                        	pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
 	...
 	inthyd = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/X6X-X7X.pdb\n"%vxi)
	...
	ctrl.write('set %s.1 connect1 %s.1.C7\n'%(vxi, vxi))
	ctrl.write('set %s name "%s"\n'%(vxi, vxi))
	ctrl.write('set %s.1 name "%s"\n'%(vxi, vxi))
	ctrl.write('set %s head %s.1.C1\n'%(vxi, vxi))
	ctrl.write('set %s tail %s.1.C7\n'%(vxi, vxi))
