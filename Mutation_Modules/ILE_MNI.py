...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/MNI.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	hydhyd = var[2]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ILE-MNI.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.15 name "CG1"\n'%vxi)
	ctrl.write('set %s.1.16 name "HG12"\n'%vxi)
	ctrl.write('set %s.1.17 name "HG13"\n'%vxi)
