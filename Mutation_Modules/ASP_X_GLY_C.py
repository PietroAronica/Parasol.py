	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HB3' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.superimposed1('HAX3', O))
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	newoxy = var[8]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ASP_X_GLY_C.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.13 name "N1"\n'%vxi)
	ctrl.write('set %s.1.14 name "H1"\n'%vxi)
	ctrl.write('set %s.1.15 name "CAX"\n'%vxi)
	ctrl.write('set %s.1.16 name "HAX2"\n'%vxi)
	ctrl.write('set %s.1.17 name "HAX3"\n'%vxi)
	ctrl.write('set %s.1.18 name "C1"\n'%vxi)
	ctrl.write('set %s.1.19 name "O1"\n'%vxi)
