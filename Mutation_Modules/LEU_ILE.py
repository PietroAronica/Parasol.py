...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HG12'.format(vxi), ':{}@HD21'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@HG13'.format(vxi), bc['HG']+((fc['HG13']-bc['HG'])/10)*i).execute()
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HB3' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.superimposed3('HG12', CD2))
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	hydhyd2 = var[5]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ILE-LEU.pdb\n"%vxi)
