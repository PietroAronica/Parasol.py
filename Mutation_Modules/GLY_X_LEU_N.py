...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@O'.format(vxi), ':{}@H1'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@HD11'.format(vxi), bc['HD12']).execute()
                change(parm, 'charge', ':{}@HD12'.format(vxi), bc['HD12']).execute()
                change(parm, 'charge', ':{}@HD13'.format(vxi), bc['HD13']).execute()
                change(parm, 'charge', ':{}@CD2'.format(vxi), bc['CD2']).execute()
                change(parm, 'charge', ':{}@HD21'.format(vxi), bc['HD22']).execute()
                change(parm, 'charge', ':{}@HD22'.format(vxi), bc['HD22']).execute()
                change(parm, 'charge', ':{}@HD23'.format(vxi), bc['HD23']).execute()
...
def makevxi(struct, out, aa, vxi='VXI', linked_to='default'):
	...
	if linked_to == 'default':
	        ...
	        C = struct.residue_dict[int(linked_to[0])].atom_dict[linked_to[1]]
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	newoxy = var[8]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/GLY_X_LEU_N.pdb\n"%vxi)
