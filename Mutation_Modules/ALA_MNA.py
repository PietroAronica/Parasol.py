...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/MNA.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	hydhyd = var[2]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ALA-MNA.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.11 name "HB2"\n'%vxi)
	ctrl.write('set %s.1.12 name "HB3"\n'%vxi)
