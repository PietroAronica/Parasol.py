...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/B2T.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	ih = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/THR-B2T.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.14 name "OG1"\n'%vxi)
	ctrl.write('set %s.1.15 name "HG1"\n'%vxi)
	...
	ctrl.write('set %s.1.14 type "OH"\n'%vxi)
	ctrl.write('set %s.1.15 type "HO"\n'%vxi)
