...
def parmed_command(vxi='VXI'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HD1 :{}@HE1 0 0'.format(vxi, vxi)).execute()
                ...
                change(parm, 'charge', ':{}@HD1'.format(vxi), bc['HE1']-((bc['HE1'])/10)*i).execute()
                change(parm, 'charge', ':{}@HD2'.format(vxi), bc['HE2']+((fc['HD2']-bc['HE2'])/10)*i).execute()
                change(parm, 'charge', ':{}@HD3'.format(vxi), bc['HE3']+((fc['HD3']-bc['HE3'])/10)*i).execute()
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'CE' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.halfway_between('CE', CE, HE1))
                        	pdb.write(atom.superimposed1('HE1', HE1))
                        	pdb.write(atom.superimposed2('HE2', HE1))
                        	pdb.write(atom.superimposed3('HE3', HE1))
			else:
...
def lib_make(ff, outputfile, vxi='VXI', estsul='ss', mthhyd='th', metcar='dc', methyd='dh', hydhyd='mh'):
        ctrl = open('lyp.in', 'w')
        ctrl.write("source leaprc.%s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/MET-NLE.pdb\n"%vxi)
...
def stock_add_to_all(estsul='ss', mthhyd='th', metcar='dc', methyd='dh', hydhyd='mh'):
	...
	for i in range(11):
		...
		Frcmod_creator.ANGLE_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}'.format(estsul, 'CT', metcar), lac(p['C_C_C'][0], p['C_C_H'][0], i), lac(p['C_C_C'][1], p['C_C_H'][1], i))
		Frcmod_creator.ANGLE_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}'.format(estsul, 'CT', hydhyd), lac(p['C_C_H'][0], p['C_C_H'][0], i), lac(p['C_C_H'][1], p['C_C_H'][1], i))
