...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HB1'.format(vxi), ':{}@HD21'.format(vxi), '0', '0').execute()
                change(parm, 'charge', ':{}@N'.format(vxi), bc['N']+((fc['N']-bc['N'])/100)*i*i).execute()
                change(parm, 'charge', ':{}@H'.format(vxi), bc['H']+((fc['H']-bc['H'])/100)*i*i).execute()
                change(parm, 'charge', ':{}@CA'.format(vxi), bc['CA']+((fc['CA']-bc['CA'])/100)*i*i).execute()
                change(parm, 'charge', ':{}@HA'.format(vxi), bc['HA']+((fc['HA']-bc['HA'])/100)*i*i).execute()
                change(parm, 'charge', ':{}@CB'.format(vxi), bc['CB']+((fc['CB']-bc['CB'])/100)*i*i).execute()
                change(parm, 'charge', ':{}@HB1'.format(vxi), bc['HB1']-(bc['HB1']/100)*i*i).execute()
                change(parm, 'charge', ':{}@HB2'.format(vxi), bc['HB2']+((fc['HB2']-bc['HB2'])/100)*i*i).execute()
                change(parm, 'charge', ':{}@HB3'.format(vxi), bc['HB3']+((fc['HB3']-bc['HB3'])/100)*i*i).execute()
                change(parm, 'charge', ':{}@CG'.format(vxi), fc['CG']/100*i*i).execute()
                change(parm, 'charge', ':{}@OD1'.format(vxi), fc['OD1']/100*i*i).execute()
                change(parm, 'charge', ':{}@ND2'.format(vxi), fc['ND2']/100*i*i).execute()
                change(parm, 'charge', ':{}@HD21'.format(vxi), fc['HD21']/100*i*i).execute()
                change(parm, 'charge', ':{}@HD22'.format(vxi), fc['HD22']/100*i*i).execute()
                change(parm, 'charge', ':{}@C'.format(vxi), bc['C']+((fc['C']-bc['C'])/100)*i*i).execute()
                change(parm, 'charge', ':{}@O'.format(vxi), bc['O']+((fc['O']-bc['O'])/100)*i*i).execute()
