...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/GAB.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@CA3'.format(vxi), (fc['CA3']/10)*i).execute()
                change(parm, 'charge', ':{}@HA32'.format(vxi), (fc['HA32']/10)*i).execute()
                change(parm, 'charge', ':{}@HA33'.format(vxi), (fc['HA33']/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
	...
	pdb = open(out, 'w')
		...
		for atom in res.atom_list:
			if atom.get_name() == 'HA23' and res.get_resname() == vxi:
				pdb.write(atom.formatted())
				pdb.write(atom.halfway_between('CA3', CA2, C)) 
				pdb.write(atom.superimposed1('HA32', C)) 
				pdb.write(atom.superimposed2('HA33', C)) 
			else:
				pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	ih = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/BAA-GAB.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.7 name "HA22"\n'%vxi)
	ctrl.write('set %s.1.8 name "HA23"\n'%vxi)
	ctrl.write('set %s.1.9 name "CA3"\n'%vxi)
	ctrl.write('set %s.1.10 name "HA32"\n'%vxi)
	ctrl.write('set %s.1.11 name "HA33"\n'%vxi)
