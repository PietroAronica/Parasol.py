...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@CZ3'.format(vxi), (bc['CZ2']-(bc['CZ2']/10)*i)*(10-i)/10).execute()
                change(parm, 'charge', ':{}@HZ3'.format(vxi), (bc['HZ2']-(bc['HZ2']/10)*i)*(10-i)/10).execute()
                change(parm, 'charge', ':{}@HZ2'.format(vxi), (bc['HZ1']-(bc['HZ1']/10)*i)*(10-i)/10).execute()
                change(parm, 'charge', ':{}@CH2'.format(vxi), (bc['CH']-(bc['CH']/10)*i)*(10-i)/10).execute()
                change(parm, 'charge', ':{}@HH2'.format(vxi), (bc['HH']-(bc['HH']/10)*i)*(10-i)/10).execute()
