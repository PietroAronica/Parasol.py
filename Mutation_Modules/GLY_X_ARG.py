...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@CA :{}@H1 0 0'.format(vxi, vxi)).execute()
		changeLJPair(parm, ':{}@O :{}@H1 0 0'.format(vxi, vxi)).execute()
                ...
                change(parm, 'charge', ':{}@NE'.format(vxi), bc['NE']).execute()
                change(parm, 'charge', ':{}@HE'.format(vxi), bc['HE']).execute()
                change(parm, 'charge', ':{}@CZ'.format(vxi), bc['CZ']).execute()
                change(parm, 'charge', ':{}@NH1'.format(vxi), bc['NH1']).execute()
                change(parm, 'charge', ':{}@HH11'.format(vxi), bc['HH11']).execute()
                change(parm, 'charge', ':{}@HH12'.format(vxi), bc['HH12']).execute()
                change(parm, 'charge', ':{}@NH2'.format(vxi), bc['NH2']).execute()
                change(parm, 'charge', ':{}@HH21'.format(vxi), bc['HH21']).execute()
                change(parm, 'charge', ':{}@HH22'.format(vxi), bc['HH22']).execute()
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'N' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.superimposed1('HA13', H))
                        	pdb.write(atom.thirdone_between('C1', N, CA))
                        	pdb.write(atom.thirdtwo_between1('O1', N, CA))
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	newoxy = var[8]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/GLY_X_ARG.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.27 element "N"\n'%vxi)
	ctrl.write('set %s.1.28 element "H"\n'%vxi)
	ctrl.write('set %s.1.29 element "H"\n'%vxi)
	ctrl.write('set %s.1.30 element "C"\n'%vxi)
	ctrl.write('set %s.1.31 element "O"\n'%vxi)
	...
	ctrl.write('set %s.1.16 name "HG2"\n'%vxi)
	ctrl.write('set %s.1.17 name "HG3"\n'%vxi)
	ctrl.write('set %s.1.18 name "CD"\n'%vxi)
	ctrl.write('set %s.1.19 name "HD2"\n'%vxi)
	ctrl.write('set %s.1.20 name "HD3"\n'%vxi)
	ctrl.write('set %s.1.21 name "NE"\n'%vxi)
	ctrl.write('set %s.1.22 name "HE"\n'%vxi)
	ctrl.write('set %s.1.23 name "CZ"\n'%vxi)
	ctrl.write('set %s.1.24 name "NH1"\n'%vxi)
	ctrl.write('set %s.1.25 name "HH11"\n'%vxi)
	ctrl.write('set %s.1.26 name "HH12"\n'%vxi)
	ctrl.write('set %s.1.27 name "NH2"\n'%vxi)
	ctrl.write('set %s.1.28 name "HH21"\n'%vxi)
	ctrl.write('set %s.1.29 name "HH22"\n'%vxi)
	ctrl.write('set %s.1.30 name "C"\n'%vxi)
	ctrl.write('set %s.1.31 name "O"\n'%vxi)
	...
	ctrl.write('set %s.1.20 type "H1"\n'%vxi)
	ctrl.write('set %s.1.21 type "N2"\n'%vxi)
	ctrl.write('set %s.1.22 type "H"\n'%vxi)
	ctrl.write('set %s.1.23 type "CA"\n'%vxi)
	ctrl.write('set %s.1.24 type "N2"\n'%vxi)
	ctrl.write('set %s.1.25 type "H"\n'%vxi)
	ctrl.write('set %s.1.26 type "H"\n'%vxi)
	ctrl.write('set %s.1.27 type "N2"\n'%vxi)
	ctrl.write('set %s.1.28 type "H"\n'%vxi)
	ctrl.write('set %s.1.29 type "H"\n'%vxi)
	ctrl.write('set %s.1.30 type "C"\n'%vxi)
	ctrl.write('set %s.1.31 type "O"\n'%vxi)
...
def stock_add_to_all(var=variablemake()):
	...
	newoxy = var[8]
        ...
        for i in range(11):
                ...
                Frcmod_creator.ANGLE_insert('{}_{}.frcmod'.format(a, 100-a), '{}-{}-{}'.format(newoxy, newcar, oldnit), cal(p['Close'][0], p['O_C_N'][0], i), cal(p['Close'][1], p['O_C_N'][1], i))
