...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/B2I.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@HD12'.format(vxi), bc['HD13']+((fc['HD13']-bc['HD13'])/10)*i).execute()
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	ih = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ILE-B2I.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.10 name "CG2"\n'%vxi)
	ctrl.write('set %s.1.11 name "HG21"\n'%vxi)
	ctrl.write('set %s.1.12 name "HG22"\n'%vxi)
	ctrl.write('set %s.1.13 name "HG23"\n'%vxi)
	ctrl.write('set %s.1.14 name "CG1"\n'%vxi)
	ctrl.write('set %s.1.15 name "HG12"\n'%vxi)
	ctrl.write('set %s.1.16 name "HG13"\n'%vxi)
