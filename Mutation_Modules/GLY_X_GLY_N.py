...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@O'.format(vxi), ':{}@H1'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@HA2'.format(vxi), bc['HA2']).execute()
                change(parm, 'charge', ':{}@HA3'.format(vxi), bc['HA3']).execute()
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	newoxy = var[8]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/GLY_X_GLY_N.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.11 name "HA2"\n'%vxi)
	ctrl.write('set %s.1.12 name "HA3"\n'%vxi)
