...
def parmed_command(vxi='VXI'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                changeLJPair(parm, ':{}@HB2 :{}@OD3 0 0'.format(vxi, vxi)).execute()
                ...
                change(parm, 'charge', ':{}@OD3'.format(vxi), (bc['OD1']-(bc['OD1']/10)*i)*(10-i)/10).execute()
                change(parm, 'charge', ':{}@OD4'.format(vxi), (bc['OD2']-(bc['OD2']/10)*i)*(10-i)/10).execute()
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'HB2' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.change_name('OD3'))
			elif atom.get_name() == 'OD2' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('OD4'))
...
def lib_make(ff, outputfile, vxi='VXI', carcar='cc', caroxy='co', hydhyd1='sh', amicar='ac', amioxy='ao', aminit='an', amihyd='ah', hydhyd2='rh'):
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/ASP-ASN.pdb\n"%vxi)
	...
	ctrl.write('set %s.1.10 name "OD3"\n'%vxi)
	ctrl.write('set %s.1.11 name "OD4"\n'%vxi)
...
def stock_add_to_all(hydhyd2='rh', serhyd='fh', carcar='cc', caroxy='co', hydhyd1='sh', amicar='ac', amioxy='ao', aminit='an', amihyd='ah'):
