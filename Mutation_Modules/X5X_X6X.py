...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@C5'.format(vxi), (fc['C5']/10)*i).execute()
                change(parm, 'charge', ':{}@H52'.format(vxi), (fc['H52']/10)*i).execute()
                change(parm, 'charge', ':{}@H53'.format(vxi), (fc['H53']/10)*i).execute()
                change(parm, 'charge', ':{}@C6'.format(vxi), bc['C5']+((fc['C6']-bc['C5'])/10)*i).execute()
                change(parm, 'charge', ':{}@H62'.format(vxi), bc['H52']+((fc['H62']-bc['H52'])/10)*i).execute()
                change(parm, 'charge', ':{}@H63'.format(vxi), bc['H53']+((fc['H63']-bc['H53'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'H43' and res.get_resname() == vxi:
                        	pdb.write(atom.formatted())
                        	pdb.write(atom.halfway_between('C5', C4, C5))
                        	pdb.write(atom.superimposed1('H52', C5))
                        	pdb.write(atom.superimposed1('H53', C5))
			elif atom.get_name() == 'C5' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('C6'))
			elif atom.get_name() == 'H52' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H62'))
			elif atom.get_name() == 'H53' and res.get_resname() == vxi:
                        	pdb.write(atom.change_name('H63'))
			else:
                        	pdb.write(atom.formatted())
	                try:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
 	...
 	inthyd = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/X5X-X6X.pdb\n"%vxi)
	...
	ctrl.write('set %s.1 connect1 %s.1.C6\n'%(vxi, vxi))
	ctrl.write('set %s name "%s"\n'%(vxi, vxi))
	ctrl.write('set %s.1 name "%s"\n'%(vxi, vxi))
	ctrl.write('set %s head %s.1.C1\n'%(vxi, vxi))
	ctrl.write('set %s tail %s.1.C6\n'%(vxi, vxi))
