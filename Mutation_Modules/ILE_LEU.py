...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HG12'.format(vxi), ':{}@HD21'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@HG'.format(vxi), bc['HG13']+((fc['HG']-bc['HG13'])/10)*i).execute()
	...
	pdb = open(out, 'w')
        ...
        for res in struct.residue_list:
			...
			if atom.get_name() == 'CB' and res.get_resname() == vxi:
                        	...
                        	pdb.write(atom.halfway_between('CD2', CG1, HG12))
                        	pdb.write(atom.superimposed1('HD21', HG12))
                        	pdb.write(atom.superimposed2('HD22', HG12))
                        	pdb.write(atom.superimposed3('HD23', HG12))
			else:
                        	pdb.write(atom.formatted())
	                try:
