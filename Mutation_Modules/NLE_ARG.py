...
def parmed_command(vxi='VXI', lipid='No'):
	...
	fc = {}
	if lipid == 'Yes':
	        with open('Param_files/AminoAcid/ARG.param.0', 'r') as b:
        	        data = b.readlines()[1:]
	else:
	...
	for i in range(11):
		...
		changeLJPair(parm, ':{}@HE2'.format(vxi), ':{}@HH21'.format(vxi), '0', '0').execute()
                ...
                change(parm, 'charge', ':{}@HE'.format(vxi), (fc['HE']/10)*i*i/10).execute()
                ...
                change(parm, 'charge', ':{}@NH1'.format(vxi), (fc['NH1']/10)*i*i/10).execute()
                change(parm, 'charge', ':{}@HH11'.format(vxi), (fc['HH11']/10)*i*i/10).execute()
                change(parm, 'charge', ':{}@HH12'.format(vxi), (fc['HH12']/10)*i*i/10).execute()
                change(parm, 'charge', ':{}@NH2'.format(vxi), (fc['NH2']/10)*i*i/10).execute()
                change(parm, 'charge', ':{}@HH21'.format(vxi), (fc['HH21']/10)*i*i/10).execute()
                change(parm, 'charge', ':{}@HH22'.format(vxi), (fc['HH22']/10)*i*i/10).execute()
