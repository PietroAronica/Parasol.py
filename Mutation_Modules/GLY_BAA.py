...
def parmed_command(vxi='VXI', lipid='No'):
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@HA12'.format(vxi), bc['HA2']+((fc['HA12']-bc['HA2'])/10)*i).execute()
                change(parm, 'charge', ':{}@HA13'.format(vxi), bc['HA3']+((fc['HA13']-bc['HA3'])/10)*i).execute()
...
def makevxi(struct, out, aa, vxi='VXI'):
	...
	pdb = open(out, 'w')
		...
		for atom in res.atom_list:
			...
			elif atom.get_name() == 'HA2' and res.get_resname() == vxi:
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	ih = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/GLY-BAA.pdb\n"%vxi)
