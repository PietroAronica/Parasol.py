...
def parmed_command(vxi='VXI', lipid='No'):
	bc = {}
        ...
        b.close()
	fc = {}
        with open('Param_files/AminoAcid/B1Q.param', 'r') as b:
                data = b.readlines()[1:]
        for line in data:
		key, value = line.split()
		fc[key] = float(value)
	...
	for i in range(11):
		...
		parm = AmberParm('Solv_{}_{}.prmtop'.format(a, 100-a))
                ...
                change(parm, 'charge', ':{}@NE2'.format(vxi), bc['NE2']+((fc['NE2']-bc['NE2'])/10)*i).execute()
                change(parm, 'charge', ':{}@HE21'.format(vxi), bc['HE21']+((fc['HE21']-bc['HE21'])/10)*i).execute()
                change(parm, 'charge', ':{}@HE22'.format(vxi), bc['HE22']+((fc['HE22']-bc['HE22'])/10)*i).execute()
...
def lib_make(ff, outputfile, vxi='VXI', var=variablemake()):
	...
	ih = var[1]
        ctrl = open('lyp.in', 'w')
        ctrl.write("source %s\n"%ff)
	ctrl.write("%s=loadpdb Param_files/LibPDB/GLN-B1Q.pdb\n"%vxi)
